...
global IonBc PhiBc HOT ChemBc dCdxBc DiffMat_cgl
...
% Compute dE/dc
dEdc = kBxT * S * (log(T * Conc) + 1) ...
    + repmat(Valence,N+1,1) .* repmat(phi,1,TotNumIon);
Mob = repmat(DiffIon,N+1,1)/kBxT .* Conc;
% ddEdc = chebfft1(dEdc,2);
% disp (size(ddEdc))
% pause
Current_PNP = - (T * Mob) .* (diag(dxi_dx)*T*chebfft1(dEdc,2));
...
dEdc_ext(1:N+1,1) = dEdc_ext(1:N+1,1) ...
    + HOT(1,1) * (- diag(1./A) * DiffMat_cgl * diag(A) * DiffMat_cgl * Conc(1:N+1,1) ...
                + S*eN ./ M(end,end) * dxi_dx(N+1) * (eN' * diag(dxi_dx)...
                * DiffMat_cgl * Conc(1:N+1,1) - dCdxBc(2,1)) ...
                - S*e0 ./ M( 1 , 1 ) * dxi_dx( 1 ) * (e0' * diag(dxi_dx)...
                * DiffMat_cgl * Conc(1:N+1,1) - dCdxBc(1,1))) ...
    + HOT(1,2) * (- diag(1./A) * DiffMat_cgl * diag(A) * DiffMat_cgl * Conc(1:N+1,2) ...
                + S*eN ./ M(end,end) * dxi_dx(N+1) * (eN' * diag(dxi_dx)...
                * DiffMat_cgl * Conc(1:N+1,2) - dCdxBc(2,2)) ...
                - S*e0 ./ M( 1 , 1 ) * dxi_dx( 1 ) * (e0' * diag(dxi_dx)...
                * DiffMat_cgl * Conc(1:N+1,2) - dCdxBc(1,2))); 
            
dEdc_ext(1:N+1,2) = dEdc_ext(1:N+1,2) ...
    + HOT(2,1) * (- diag(1./A) * DiffMat_cgl * diag(A) * DiffMat_cgl * Conc(1:N+1,1) ...
                + S*eN ./ M(end,end) * dxi_dx(N+1) * (eN' * diag(dxi_dx) ...
                * DiffMat_cgl * Conc(1:N+1,1) - dCdxBc(2,1)) ...
                - S*e0 ./ M( 1 , 1 ) * dxi_dx( 1 ) * (e0' * diag(dxi_dx) ...
                * DiffMat_cgl * Conc(1:N+1,1) - dCdxBc(1,1))) ...
    + HOT(2,2) * (- diag(1./A) * DiffMat_cgl * diag(A) * DiffMat_cgl * Conc(1:N+1,2) ...
                + S*eN ./ M(end,end) * dxi_dx(N+1) * (eN' * diag(dxi_dx) ...
                * DiffMat_cgl * Conc(1:N+1,2) - dCdxBc(2,2)) ...
                - S*e0 ./ M( 1 , 1 ) * dxi_dx( 1 ) * (e0' * diag(dxi_dx) ...
                * DiffMat_cgl * Conc(1:N+1,2) - dCdxBc(1,2))); 
...
Current_ext = - (T*Mob) .* (diag(dxi_dx)*T*chebfft1(dEdc_ext,2));
I_ext = zeros(size(sum(repmat(ValIon,N+1,1) .* Current_ext,2)));

dEdc = dEdc+dEdc_ext;
ddEdc = T*chebfft1(dEdc,2);
Current = - (T*Mob) .* (diag(dxi_dx)*ddEdc);%*chebfft1(dEdc,2));
...
DivCurrent = - diag(1./(Jacobian.*Area)) ...
    * (chebfft1(diag(Jacobian .* Area .* dxi_dx) * S * Current,2));
% DivCurrent = - diag(1./(Jacobian.*Area)) ...
...
for IonNum=1:TotNumIon
        ...
        g_m = 0; g_p = 0; 
disp([size(e0' * T * Mob) size(e0' * T * dEdc)])
...
% %         * ((e0' * T * Mob) * (e0' * T * dEdc) - g_m) ...
% %         - AlphaBetaIon(1,2,IonNum) * (-Current( 1 ,IonNum) - 0) );
% % 
% %     r_p = Area(N+1) * (AlphaBetaIon(2,1,IonNum) ...
% %         * ((eN' * T * Mob) * (eN' * T * dEdc) - g_p) ...
% %         + AlphaBetaIon(2,2,IonNum) * (-Current(N+1,IonNum) - 0) );
    
    % add penalty BC to flux
% disp([lc_p lc_m])
% pause
    DivCurrent(:,IonNum) = DivCurrent(:,IonNum) - TauIon(1,IonNum) * S*e0/M(1,1) .* r_m ...
                            - TauIon(2,IonNum) * S * eN/M(end,end) .* r_p;
