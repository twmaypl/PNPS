...
global AlphaBetaIon AreaStr IonBc PhiBc ddxAdCdxBc ChemBc dCdxBc 
...
ep=p.ep; DiffIon = p.DiffIon; AreaStr = p.AreaStr; epslion = p.epslion; 
IonBc = p.IonBc; PhiBc = p.PhiBc; dt=p.dt;
xmin = p.xmin; xmax = p.xmax;
ddxAdCdxBc = p.ddxAdCdxBc; ChemBc = p.ChemBc; dCdxBc = p.dCdxBc; 
...
for IndxStericG =5:5 %1:length(TotStericG(1,1,:))
    StericG = TotStericG(:,:,IndxStericG)
    for IndxHOT = 2:2% 1:length(TotHOT)
        HOT = TotHOT(:,:,IndxHOT)
for i = 1:length(NN)
    N = NN(i);
    ModelPnP_PN_ODE15s(dt);%(IonBc, PhiBc);
%     [err(i,j)]= ModelPnP_PN(IonBc,PhiBc);
end
    end
