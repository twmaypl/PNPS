...
global IonBc PhiBc ddxAdCdxBc dCdxBc 
...
dx_min=abs(x(2)-x(1));
% dt = CFL * 0.1/(max(DiffIon)) *dx_min^2;
...
global IonBc PhiBc ddxAdCdxBc HOT ChemBc dCdxBc 
global kBxT TotNumIon StericG DiffMat ValIon e_unit
...
% Compute the first derivative boundary condition dCdxBc  

% % % psi (1:N+1,1) = dEdc(1:N+1,1) + HOT(1,1) * (- diag(1./A) * DiffMat ...
% % %                  * diag(A) * DiffMat * Conc(1:N+1,1)) ...
% % %                  + HOT(1,2) * (- diag(1./A) * DiffMat ...
% % %                  * diag(A) * DiffMat * Conc(1:N+1,2));
% % % psi (1:N+1,2) = dEdc(1:N+1,2) + HOT(2,1) * (- diag(1./A) * DiffMat ...
% % %                  * diag(A) * DiffMat * Conc(1:N+1,1)) ...
% % %                  + HOT(2,2) * (- diag(1./A) * DiffMat ...
% % %                  * diag(A) * DiffMat * Conc(1:N+1,2));
% % %              
% % % Mob = repmat(DiffIon,N+1,1)/kBxT .* Conc;
% % % dCdxBc(1,1:2) = -Mob(1,1:end) .* ((e0' * DiffMat*DiffMat*psi)./(e0' *DiffMat*psi));
% % % dCdxBc(2,1:2) = -Mob(N+1,1:end) .* ((eN' * DiffMat*DiffMat*psi)./(eN' *DiffMat*psi));

% dpsidx = DiffMat * psi; ddpsidx = DiffMat * dpsidx;
% dCdx = - Mob .* ddpsidx ./ dpsidx;
% dCdxBc = [dCdx(1,1) dCdx(1,2); dCdx(end,1) dCdx(end,2)]; 
% disp(dCdxBc)
% pause

    
%add High order term
% % dEdc(1:N+1,1) = dEdc(1:N+1,1) ...
% %     + HOT(1,1) * (- diag(1/Area) * DiffMat * diag(Area) * DiffMat * Conc(1:N+1,1));
% % dEdc(1,1)   = HOT(1,1)*( - 1/M(1,1) * (DiffMat(1,1:N+1) * Conc(1:N+1,1) - dCdxBc(1,1)));
% % dEdc(N+1,1) = HOT(1,1)*(   1/M(end,end) * (DiffMat(N+1,1:N+1) * Conc(1:N+1,1) - dCdxBc(2,1)));
...
dEdc_ext(1:N+1,1) = dEdc_ext(1:N+1,1) ...
    + HOT(1,1) * (- diag(1./A) * DiffMat * diag(A) * DiffMat * Conc(1:N+1,1) ...
                ...
                * DiffMat * Conc(1:N+1,1) - dCdxBc(2,1)) ...
                - e0 / M( 1 , 1 ) * dxi_dx( 1 ) * (e0' * diag(dxi_dx)...
                * DiffMat * Conc(1:N+1,1) - dCdxBc(1,1))) ...
    + HOT(1,2) * (- diag(1./A) * DiffMat * diag(A) * DiffMat * Conc(1:N+1,2) ...
                + eN / M(end,end) * dxi_dx(N+1) * (eN' * diag(dxi_dx)...
                * DiffMat * Conc(1:N+1,2) - dCdxBc(2,2)) ...
                - e0 / M( 1 , 1 ) * dxi_dx( 1 ) * (e0' * diag(dxi_dx)...
                * DiffMat * Conc(1:N+1,2) - dCdxBc(1,2))); 
